...
class MLP(nn.Module):
    def __init__(self, state_size, hidden_size, action_size):
        ...
        # 定义Dueling DQN的多层感知机结构
        self.fc1 = nn.Linear(state_size, hidden_size)
        self.fc2 = nn.Linear(hidden_size, hidden_size)
        self.v = nn.Linear(hidden_size, 1)
        self.q = nn.Linear(hidden_size, action_size)
...
class DuelingDQN(object):
    ...
    def save(self, path, episode):
        # 保存模型和优化器的状态字典
        torch.save(self.model.state_dict(), "{}/dueling_dqn_{}".format(path, episode))
        torch.save(self.optimizer.state_dict(), "{}/dueling_dqn_optimizer_{}".format(path, episode))

    def load(self, path, episode):
        # 加载模型和优化器的状态字典
        self.model.load_state_dict(torch.load("{}/dueling_dqn_{}".format(path, episode)))
        self.optimizer.load_state_dict(torch.load("{}/dueling_dqn_optimizer_{}".format(path, episode)))
        self.target_model.load_state_dict(self.model.state_dict())
