...
import torch
import numpy as np
from CQL_SAC import CQL_SAC
from utils import ReplayBuffer


# 定义训练 CQL_SAC 代理的函数
def train_cql_sac(params):
    ...
    # 初始化 CQL_SAC 代理
    policy = CQL_SAC(state_dim, action_dim, max_action, hidden_size=32)
...
def eval_policy(policy, env_name, training_iters, eval_episodes=3):
    ...
    for _ in range(eval_episodes):
        ...
        step = 0
        while not done:
            # 根据 CQL_SAC 代理选择动作并与环境交互
            action = policy.predict(np.array(state), noise=False)
...
param = {
    ...
    "save_path": "CQL_SAC_Pendulum",  # 模型保存路径
